      ...
      <div className="flex my-6 items-center">
        <InView>
          {({ inView, ref, entry }) => (
            ...
            >
                 <h1 className='text-black absolute top-1/2 transform -translate-y-1/2 z-40 right-1/4'>{followers} followers</h1>
              {inView && (
                <motion.div
                  initial={{ x: -400 }}
                  animate={{ x: 0 }}
      ...
      <div className="flex my-6 items-center">
        <InView>
          {({ inView, ref, entry }) => (
            ...
            >
                <h1 className='text-black absolute top-1/2 transform -translate-y-1/2 z-40 right-1/4'>{repos} Repositories</h1>
              {inView && (
                <motion.div
                  initial={{ x: -400 }}
                  animate={{ x: 0 }}
        ...
        <InView>
          {({ inView, ref, entry }) => (
            <div
              ...
              className="instagram relative h-16 rounded-xl w-10/12 bg-black-200 overflow-hidden"
            >
                 <h1 className='text-black absolute top-1/2 transform -translate-y-1/2 z-40 right-1/4'>{subscribers} Subscribers</h1>
              {inView && (
                <motion.div
                  initial={{ x: -400 }}
                  animate={{ x: 0 }}
